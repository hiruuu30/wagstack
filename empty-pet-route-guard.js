(()=>{
  ...
  function renderEmpty(path,push=true){
    ...
    main.innerHTML=`<section class="clone-page"><header class="clone-page__head"><div><div class="clone-kicker">${health?'HEALTH & CARE':'MY PETS'}</div><h1>${health?'Health records start with a pet.':'Add your first pet.'}</h1><p>${health?'Create a pet profile first, then vaccinations, preventives, care notes and other records will live here.':'Create a real pet profile to start bookings, health records, care notes and rewards without any demo data.'}</p></div></header><div class="clone-glass"><article class="clone-card clone-card--full" style="text-align:center;padding:clamp(28px,6vw,64px)"><div class="clone-card__no">EMPTY PAWFILE</div><h2 style="margin:8px 0 10px">${health?'No pet selected yet':'Your Pawfile is ready'}</h2><p style="max-width:520px;margin:0 auto 18px">${health?'Add a pet and WagStack will create a clean Health & Care workspace for that pet.':'There are no placeholder pets in production. Add your own pet to begin.'}</p><button class="clone-btn" type="button" data-add-pet-main>+ Add pet</button></article></div></section>`;
    setActive(path);
  }
