...
(()=>{
  ...
  function render(){
    ...
    const orderRows=orders.map(o=>`<tr><td><strong>${esc(String(o.id).slice(0,8).toUpperCase())}</strong><div class="wa-owner">${fmtDate(o.created_at)}</div></td><td>${esc(o.customer)}</td><td>${esc(o.items.map(i=>`${i.product_name||'Item'} ×${i.quantity}`).join(', ')||'—')}</td><td>${money(o.total)}</td><td><select class="wa-select" aria-label="Order status for ${esc(o.customer)}" data-order-status="${esc(o.id)}">${['pending','paid','processing','ready','completed','cancelled','refunded'].map(s=>`<option value="${s}" ${s===o.status?'selected':''}>${statusLabel(s)}</option>`).join('')}</select></td></tr>`);
  ...
  document.addEventListener('click',e=>{
    if(location.pathname!==PATH)return;
    const tab=e.target.closest('[data-wa]');if(tab){activeTab=tab.dataset.wa;document.querySelectorAll('[data-wa]').forEach(x=>x.classList.toggle('is-on',x===tab));document.querySelectorAll('.wa-view').forEach(v=>v.classList.toggle('is-on',v.dataset.view===tab.dataset.wa))}
