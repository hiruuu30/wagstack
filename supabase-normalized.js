...
async function syncMembership(owner,state){
  ...
  if(!m.active){await cloud().db.delete('user_memberships','legacy_key=eq.primary');return}
...
async function syncCartOrder(owner,state){
  ...
  if(!cart.length){await cloud().db.delete('orders','legacy_key=eq.active-cart');return {orders:0,items:0}}
  const products=await cloud().db.get('products','select=id,legacy_key,name,price&active=eq.true');const byKey=new Map(products.map(p=>[String(p.legacy_key||''),p]));const counts=new Map();
  ...
  if(!orderRows.length)orderRows=await cloud().db.get('orders','select=id&legacy_key=eq.active-cart&limit=1');const order=orderRows[0];if(!order)return {orders:0,items:0};
...
async function syncNormalized({markReady=false}={}){
  if(syncing||hydrating||!uid()||!cloud()?.db)return false;syncing=true;cloud()?.setCloudState?.('syncing');
  try{
    ...
    const petRows=await upsert('pets',pets,'owner_id,legacy_key');const existingPets=petRows.length?petRows:await cloud().db.get('pets','select=id,name,legacy_key');const petByKey=new Map(existingPets.map(p=>[String(p.legacy_key||''),p]));const petByName=new Map(existingPets.map(p=>[String(p.name||''),p]));await removeMissing('pets','legacy_key',new Set(pets.map(p=>p.legacy_key)));
    const health=[];
  ...
  }catch(err){console.error('[WagStack normalized sync]',err);cloud()?.setCloudState?.('error');return false}finally{syncing=false}
}
...
async function hydrateFromSupabase(){
  ...
  try{
    ...
    const [profiles,wallets,memberships,pets,health,bookings,orders]=await Promise.all([
      ...
      cloud().db.get('user_memberships','select=status,starts_at,ends_at,legacy_key,memberships(name)&legacy_key=eq.primary&limit=1'),
      cloud().db.get('pets','select=id,legacy_key,name,species,breed,sex,age_label,weight_label,weight_kg,avatar_url,pixel_avatar_url,notes&order=created_at.asc'),
      cloud().db.get('health_records','select=id,pet_id,legacy_id,record_type,title,details,status,tone,record_date,expires_on,attachment_url&order=created_at.asc'),
      cloud().db.get('bookings','select=id,pet_id,legacy_id,booking_type,service_name,starts_at,ends_at,status,special_requests,total_amount,date_label,time_label&order=starts_at.asc'),
      cloud().db.get('orders','select=id,status,legacy_key&legacy_key=eq.active-cart&status=eq.pending&limit=1')
    ]);
  ...
  }catch(err){console.error('[WagStack database hydrate]',err);cloud()?.setCloudState?.('error');return false}finally{hydrating=false}
}
...
async function initializeSourceOfTruth(){
  ...
  try{
    ...
    if(!ready){const migrated=await syncNormalized({markReady:true});if(!migrated)return false;window.dispatchEvent(new CustomEvent('wagstack:database-migrated'));return false}
    return await hydrateFromSupabase();
  }finally{booting=false}
}
function schedule(){clearTimeout(syncTimer);syncTimer=setTimeout(()=>syncNormalized(),120)}
Storage.prototype.setItem=function(key,value){const out=nativeSetItem.call(this,key,value);if(this===localStorage&&key===STORE_KEY&&!hydrating&&!booting)schedule();return out};
