...
async function signUp(email,password,fullName=''){
  const r=await fetch(`${SUPABASE_URL}/auth/v1/signup`,{method:'POST',headers:headers(),body:JSON.stringify({email,password,data:{full_name:fullName}})});
...
function openAuth(){
  ...
  wrap.innerHTML=signed?`<div class="wag-auth"><button class="wag-auth-close" aria-label="Close">×</button><h2>WagStack Cloud</h2><p>Supabase is the source of truth for your WagStack account. Local storage is only used as a fast device cache.</p><div class="wag-auth-account">${escapeHtml(session.user.email||'Signed in')}</div><div class="wag-auth-actions"><button class="wag-auth-secondary" data-sync>Refresh data</button><button class="wag-auth-danger" data-signout>Sign out</button></div><p class="wag-auth-msg"></p></div>`:`<div class="wag-auth"><button class="wag-auth-close" aria-label="Close">×</button><h2>WagStack Cloud</h2><p>Sign in to load and save pet profiles, health records, bookings, rewards, shop activity and membership directly from Supabase.</p><label>Name</label><input name="name" autocomplete="name" placeholder="Fur parent name"><label>Email</label><input name="email" type="email" autocomplete="email" placeholder="you@example.com"><label>Password</label><input name="password" type="password" autocomplete="current-password" placeholder="At least 6 characters"><div class="wag-auth-actions"><button class="wag-auth-primary" data-signin>Sign in</button><button class="wag-auth-secondary" data-signup>Create account</button></div><p class="wag-auth-msg"></p></div>`;
  ...
  wrap.querySelector('[data-signup]')?.addEventListener('click',async()=>{try{msg.textContent='Creating account…';const data=await signUp(wrap.querySelector('[name=email]').value.trim(),wrap.querySelector('[name=password]').value,wrap.querySelector('[name=name]').value.trim());msg.textContent=data.access_token?'Account created. Loading your WagStack data…':'Account created. Check your email to confirm, then sign in.';if(data.access_token)setTimeout(()=>location.reload(),350)}catch(err){msg.textContent=err.message}});
...
window.WagStackSupabase={url:SUPABASE_URL,signIn,signUp,signOut,ensureSession,setCloudState,get session(){return session},db:{get:getJson,insert:insertJson,update:patchJson,delete:deleteJson}};
